...
custer_19 <- terra::rast("D:\\MT_Spatial_Data\\MT_LiDAr\\CUSTER_2019_CUcntyQL1\\Hillshade.tif")
plot(custer_19, main = "Musselshell Hillshade")
crs(custer_19) <- "EPSG:32100"
crs(custer_19)
custer_19_test <- resample(custer_19, lcov_miso, method = "near")
...
```{r Musselshell}
# select the points in the land cover we want
...
## need to make it into an sf object first?
miso_main_sf <- miso_main_trim %>% st_as_sf(coords=c("lon_WGS84", "lat_WGS84")) %>% ## Note that the order is longitude>latitude
  st_set_crs(32100)
plot(lcov_miso)
