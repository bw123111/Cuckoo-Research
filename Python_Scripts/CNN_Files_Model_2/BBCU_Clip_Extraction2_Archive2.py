...
#if exists(big_folder)==False:
#            subprocess.check_call('mkdir '+big_folder, shell=True)
...
for point in point_list:
    ...
    for cl in classes:
        ...
        #if not os.path.exists(clips_path+dataset+'_topclip_perperiod/'+point+'/'+cl):
        #    os.makedirs(clips_path+dataset+'_topclip_perperiod/'+point+'/'+cl)
        #if exists(clips_path+dataset+'_topclip_perperiod/'+point+'/'+cl)==False:
        #    subprocess.check_call('mkdir '+clips_path+dataset+'_topclip_perperiod/'+point+'/'+cl, shell=True)
        ...
        ### OLD: sub_df1 = sub_df1.groupby(['date', 'time_period'], as_index=False)[cl].max()
        #print(result_df)
        # NEED TO CHECK IF THIS WORKS ON MULTIPLE DAYS

        num += len(sub_df)
        # Make a column for clip
        keep_df['clip'] = [clips_path + dataset + '_topclip_perperiod/' + point + '/' + cl + '/' + str(keep_df['date'].iat[i]) + '_' + str(keep_df['hour'].iat[i]) + '_' + str(keep_df['start_time'].iat[i]) + 's-' + str(keep_df['end_time'].iat[i]) + 's.wav' for i in range(len(keep_df))]
        # append the result to dataframe with top scoring clips
        keep_df = pd.concat([keep_df,sub_df])
        #print('Data to keep: the first keep_df')
        #print(keep_df)
    ...
    #keep_df['clip'] = [clips_path+dataset+'_topclip_perperiod\\'+point+'\\'+cl+'\\'+keep_df['date'].iat[i]+'_'+keep_df['hour'].iat[i]+'_'+str(keep_df['star
    
    # OLD clips_path+dataset+'_topclip_perperiod\\'+point+'\\'+cl+'\\'+keep_df['file'].iat[i].split('\\')[-2]
    #print(keep_df.columns)
    #print("Data to keep:")
    #print(sub_df)
    
    # Reshape this data to create one column for call_type and one column for scores
    keep_df = pd.melt(keep_df, id_vars = ['point_id','date','hour','time_period','file','start_time','end_time','clip'], var_name = 'call_type', value_name = 'score')
    # Remove the NAs
    ...
    for i in range(len(keep_df)):
        ...
        filename = keep_df['file'].iat[i] # Changed from file to clip
        #filename = audio_path + "\\" + "\\".join(filename.split("\\")[1:])
        filename = "E:/" + "/".join(filename.split("/")[1:])
        # Check
    ...
    keep_df.to_csv(clips_path+dataset+'_topclip_perperiod/'+point+'/'+point+'_topclipsperperiod.csv')
...
for point in point_list:
    ...
    if point not in locs_list:
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
            # Might to go in and specify shell = TRUE?
    ...
    for cl in classes:
        ...
        num += len(sub_df)
        # Make a column for clip
        ## Removed + '/' + cl after point
        keep_df['clip'] = [clips_path + dataset + '_topclip_perperiod/' + point + '/' + str(keep_df['date'].iat[i]) + '_' + str(keep_df['hour'].iat[i]) + '_' + str(keep_df['start_time'].iat[i]) + 's-' + str(keep_df['end_time'].iat[i]) + 's.wav' for i in range(len(keep_df))]
        # append the result to dataframe with top scoring clips
        keep_df = pd.concat([keep_df,sub_df])
        #print('Data to keep: the first keep_df')
        #print(keep_df)
