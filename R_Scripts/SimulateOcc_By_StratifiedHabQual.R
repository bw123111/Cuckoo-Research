############### Script for Simulating Occupancy Data with Habitat Quality Stratified into Good and Poor ##########

# This is a code to simulate occupancy data that varies with habitat quality
## This script creates a function to simulate the specified number of reps at each survey size, only selecting for habitat of very good or very poor quality 
...
simulate_data_stratified <- function(sample_sizes,nreps,possible_vals){
  ...
  for (s in 1:length(sample_sizes)){
    ...
    for (q in 1:nreps){
      ...
      for (i in 1:sample_sizes[s]){
        # randomly select a row number, weighted towards the front of the back of it -  this correlates to the habitat quality and occupancy prob of mock point you're surveying
        row_num <- sample(c(1,2,3,8,9,10),1)
