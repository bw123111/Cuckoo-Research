...
#st_write(proj_hydro,".\\Data\\Spatial_Data\\MT_Rivers\\Project_Rivers.shp")
# create a point file to label the rivers
...
Fig1_base <- ggplot()+
  ...
  annotation_north_arrow(location = "bl", width=unit(.8,"cm"),height=unit(.8,"cm"),which_north = "true",pad_y = unit(0.2, "in"),style = north_arrow_fancy_orienteering) + theme(legend.position='none')#+ scale_y_continuous(limits = c(45.0,48.0),breaks = seq(45.0, 48.0, by = .5))
# not doing anything
...
# Other map: with basemap from stamenmaps
crs(basemap_orig)
plot(basemap_orig)
plot(buff, add= TRUE)
str(basemap_orig)
