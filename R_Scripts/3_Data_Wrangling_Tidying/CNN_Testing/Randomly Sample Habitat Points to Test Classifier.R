#### Randomly Sample Habitat Points to Test Classifier ###################

## Purpose: to randomly sample acoustic files to use to test the classifier
# want a representative sample of files from each of the 7 habitat types
...
# Metadata
metadat <- read.csv("./Data/Metadata/Outputs/2023_ARUDeployment_MetadataFull_Cleaned10-5.csv")
...
# Mixed forest
#m_for <- miso_hab %>% filter(landcover_strata == "Mixed Forest") %>% sample_n(1)
## Only mixed forest is along Yellowstone and Musselshell

# Woody Wetlands
wood_wet <- miso_hab %>% filter(landcover_strata == "Woody Wetlands") %>% sample_n(1)

# Emergent Herbaceous Wetlands
herb_wet <- miso_hab %>% filter(landcover_strata == "Emergent Herbaceous Wetlands") %>% sample_n(1)

# Deciduous Forest
d_for <- miso_hab %>% filter(landcover_strata == "Deciduous Forest") %>% sample_n(1)
...
# write this to a .csv
write.csv(sampled_dat, "./Data/Wrangling_CNN_Train_Test_Data/Outputs/Habitat_Points_For_Classifier_Testing.csv")
