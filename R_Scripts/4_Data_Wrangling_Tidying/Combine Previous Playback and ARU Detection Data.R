...
# add on lat and long
# Read in metadata
r7_metadat <- read_csv("./Data/Metadata/Outputs/2022_ARUDeployment_MetadataReduced_FWPR7.csv") %>% clean_names() %>% rename(long = longitude, lat = latitude) %>% select(point_id, aru_id, lat, long)
...
#### Rbind all together and export for use in arcpro ####################3
